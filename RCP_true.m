function [RCP] = RCP_true(kappa,X,Delta,BSLocationX,BSLocationY)
%RCP Summary of this function goes here
...
%                                     (exp(-mu*(u^alpha).*(2.^(rho.*N/(delta*W)) -1)*(sigma+c)))* Temp1,N,0,Inf));
            ...
            T5=trapz(T4);
            r_pdf=T5*exp(-Ab*lambda)*log(2)/A/delta/W;
